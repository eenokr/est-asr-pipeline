...
process to_json {    
    ...
    output:
    path "unpunctuated.json", emit: unpunctuated_json
...
process final_output {
    ...
    output:
    path "result.json", emit: result_json
    path "result.srt", emit: result_srt
...
workflow {
    ...
    // Prepare initial data directory
    ...
    mfcc(language_id.out.datadir, to_wav.out.audio)
    
    // Speaker identification
    speaker_id(language_id.out.datadir, to_wav.out.audio, file("$projectDir/conf"))
    
    // One-pass decoding
    one_pass_decoding(mfcc.out.datadir_hires, mfcc.out.ivectors, file("$projectDir/conf"))
    
    // RNNLM rescoring
    rnnlm_rescoring(one_pass_decoding.out.pruned_unk, mfcc.out.datadir_hires)
    
    // Convert lattice to CTM
    lattice2ctm(rnnlm_rescoring.out.pruned_rnnlm_unk, mfcc.out.datadir_hires)
    
    // Convert to JSON
    to_json(lattice2ctm.out.segmented_ctm, lattice2ctm.out.with_compounds_ctm, speaker_id.out.sid_result, diarization.out.show_uem_seg)
    
    // Add punctuation
    punctuation(to_json.out.unpunctuated_json)
