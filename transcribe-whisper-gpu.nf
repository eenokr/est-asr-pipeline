    ...
    shell:
        if (params.do_speaker_id)
          ...
          spk_id.sh --device 1 --accelerator gpu --load-checkpoint !{params.rootdir}/models/spk_id/version_295816/checkpoints/last.ckpt \
	        ...
	        python3 -c \'import json, sys; spks={s.split()[0]:{"name" : " ".join(s.split()[2:])} for s in sys.stdin}; json.dump(spks, sys.stdout);\' > sid-result.json
          '''
        else
          '''
...
process prepare_data_for_whisper {
    ...
    output:
      path 'datadir_25sec', optional: true, emit: datadir_25sec
...
process decode_whisper {
    ...
    shell:
      '''
      #python3 -c "import torch; print(torch.cuda.is_available()); print(torch.backends.cudnn.version())"
...
process postprocess_whisper {
    ...
    output:
      path 'datadir_whisper', optional: true, emit: datadir_whisper
...
process final_output {
    ...
    shell:
      ...
      json2text.py --speaker-labels result.json > result.txt
